...
public class ShopUpPageServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int shopAllPage = (Integer) hs.getAttribute("shopAllPage");
			   if(shopCurrentPage <= 1 ){//Ê×Ò³
			      List<Shop> shopList = managerService.PagingQuery(0);
			      hs.setAttribute("shopCurrentPage", shopCurrentPage);
			      request.setAttribute("shopList", shopList);
			      request.getRequestDispatcher("WEB-INF/jsp/ShopQuery.jsp").forward(request, response);
			   }else{
				  List<Shop> shopList = managerService.PagingQuery(shopCurrentPage - 2);
